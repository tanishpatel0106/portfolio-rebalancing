...
def render_page(ctx) -> None:
    ...
    with col_cfg:
        with st.form("risk_config_form"):
            ...
            submit = st.form_submit_button("Compute Baseline Risk")
    ...
    kpi_cards = [
        styles.kpi_card("Ann. Return", styles.fmt_pct(metrics["Annualized Return"] * 100), col_span=2),
        styles.kpi_card("Ann. Vol", styles.fmt_pct(metrics["Annualized Volatility"] * 100), col_span=2),
        styles.kpi_card("VaR", styles.fmt_pct(metrics["VaR (alpha)"] * 100), col_span=2),
        styles.kpi_card("CVaR", styles.fmt_pct(metrics["ES (alpha)"] * 100), col_span=2),
        styles.kpi_card("Sharpe", styles.fmt_float(metrics["Sharpe Ratio"], decimals=2), col_span=2),
        styles.kpi_card("Max Drawdown", styles.fmt_pct(metrics["Max Drawdown ($)"] * 100), col_span=2),
    ]
    ...
    with tabs[1]:
        ...
        corr = payload["returns_df"].corr().fillna(0.0)
    ...
    with tabs[3]:
        pr = payload["portfolio_series"].replace([np.inf, -np.inf], np.nan).dropna()
