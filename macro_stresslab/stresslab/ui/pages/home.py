...
from stresslab.ui.components import kpi_row, KPI
...
def render_page(ctx) -> None:
    ...
    kpis = [
        KPI(label="Portfolios", value=len(portfolios)),
        KPI(label="Scenarios", value=len(scenarios)),
        KPI(label="Runs", value=len(runs)),
        KPI(label="Latest Run", value=latest_run_name, sub=latest_run_ts),
    ]
    kpi_row(kpis)
