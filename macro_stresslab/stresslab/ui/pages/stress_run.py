...
def render_page(ctx) -> None:
    ...
    with col_config:
        ...
        with st.form("stress_run_form"):
            ...
            submit = st.form_submit_button("🚀 Run Stress Test")
    ...
    kpi_cards = [
        styles.kpi_card("Ann. Vol", styles.fmt_pct(metrics["Annualized Volatility"] * 100), col_span=3),
        styles.kpi_card("VaR", styles.fmt_pct(metrics["VaR (alpha)"] * 100), col_span=3),
        styles.kpi_card("CVaR", styles.fmt_pct(metrics["ES (alpha)"] * 100), col_span=3),
        styles.kpi_card("Max Drawdown", styles.fmt_pct(metrics["Max Drawdown ($)"] * 100), col_span=3),
    ]
    ...
    with tabs[1]:
        ...
        if scenario_payload is None:
            ...
            if pnl_rows.empty:
                ...
                pnl_rows = pnl_rows.sort_values("ImpactValue")
                fig = px.bar(
                    pnl_rows,
                    x="Scenario",
                    y="ImpactValue",
